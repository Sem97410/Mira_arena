...
func _physics_process(delta: float) -> void:
	...
	if direction.length() > 0.1:  # **Évite les mouvements parasites**
		...
		#print(" Root motion position", animation_tree.get_root_motion_position()  )
...
func set_blend_position_amount(input_strength: float) -> void:
	animation_tree.set("parameters/StateMachine/BlendSpace1D/blend_position", input_strength)
